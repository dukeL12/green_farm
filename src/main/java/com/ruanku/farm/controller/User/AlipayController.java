...
import com.ruanku.farm.config.AlipayConfig;
...
public class AlipayController {
    ...
    @RequestMapping("/pay")
    public void pay(HttpSession session,HttpServletRequest request, HttpServletResponse response, String money, int ProOrd_id) throws Exception {

        ProOrd selProListById=proOrdServiceInter.user_selProIdById(ProOrd_id);
        String dingdanhao =String.valueOf(ProOrd_id);
        String miaosu = String.valueOf(selProListById.getPro_id());
        System.out.println(selProListById.getPro_id());
        Produce SelproduceById=produceServiceInter.user_SelproduceById(selProListById.getPro_id());
        session.setAttribute("pay_ProOrd_id", ProOrd_id);


        // 模拟从前台传来的数据
        String orderNo = dingdanhao; // 生成订单号
        String totalAmount = SelproduceById.getPro_price(); // 支付总金额
        String subject = selProListById.getProOrd_name(); // 订单名称
        String body = miaosu; // 商品描述
    ...
    public ModelAndView returnUrl(HttpSession session,HttpServletRequest request) throws Exception {
        ...
        if (signVerified) {
            ...
            int ProOrd_id= (int) session.getAttribute("pay_ProOrd_id");
